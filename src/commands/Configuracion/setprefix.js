module.exports = {
    ...
    if(args[0] == "add"){
    if(!args[1]) return message.channel.send('> Necesitas poner un prefix **'+message.author.username+'**!')
    if(args[1].length > 3) return message.channel.send('> El prefix no puede tener +3 caracteres!')
    ...
    await NewPrefix.updateOne({guildID: message.guild.id, prefix: args[1], guildName: message.guild.name})
    
    message.channel.send("> El prefix se ha establecido a " + "``" + args[1] + "``")
    
    } else {
    
    let NewPrefix2 = new ModelPrefix({guildID: message.guild.id, prefix: args[1]})
    ...
    message.channel.send("> Se ha removido el prefix actual. Se ha establecido al principal: \`\`d!\`\`")
  }
 }
}