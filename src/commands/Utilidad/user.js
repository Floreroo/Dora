...
module.exports = {
  ...
  async run(client, message, args) {
    ...
    const owo = message.mentions.users.first() || client.users.cache.get(args[0]) || client.users.cache.find(x => x.username === args[0]) || client.users.cache.find(x => x.tag === args[0]) || message.author;
    ...
    try {
      const uo = new Discord.MessageEmbed()
        .setAuthor(owo.tag, owo.displayAvatarURL({
          dynamic: true
        }))
        .addField('Ping', "\n" + owo.toString(), true)
        ...
        .addField('Insignias', `${owo.flags.toArray().length < 1 ? `No tiene` : owo.flags.toArray().map(e => badges[e]).join(' ')}`, true)
        ...
        .addField(`Ha entrado en ${server.name}`, miembro.joinedAt.toLocaleString("es"), true)
